...
// 隣接ノードを取得
export function getNeighbors(grid, node, cols, rows) {
  const neighbors = [];
  const { x, y } = node;
...
// A* アルゴリズム
export async function astar(grid, startNode, endNode, cols, rows, onVisit, onPath) {
  const openSet = [];
  startNode.g = 0;
...
// Dijkstra法
export async function dijkstra(grid, startNode, endNode, cols, rows, onVisit, onPath) {
  const openSet = [];
  startNode.g = 0;
  openSet.push(startNode);
...
// 幅優先探索 (BFS)
export async function bfs(grid, startNode, endNode, cols, rows, onVisit, onPath) {
  const queue = [];
...
// 深さ優先探索 (DFS)
export async function dfs(grid, startNode, endNode, cols, rows, onVisit, onPath) {
  const stack = [];
  startNode.visited = true;
...
export async function greedy(grid, startNode, endNode, cols, rows, onVisit, onPath) {
  const openSet = [];
